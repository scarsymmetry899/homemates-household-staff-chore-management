...
const PayrollPage = () => {
  ...
  return (
    <PageTransition className="px-6 space-y-6">
      ...
      <AnimatedCard delay={0.1} className="estate-gradient rounded-xl p-6 space-y-3">
      ...
      <StaggerContainer className="space-y-3 pb-4">
        ...
        {staff.map((s) => (
          <StaggerItem key={s.id}>
            <PressableCard className="bg-card rounded-xl p-5 shadow-card space-y-3">
              <div className="flex items-center gap-3">
                <img src={s.photo} alt={s.name} className="w-10 h-10 rounded-full object-cover" loading="lazy" />
                <div>
